...
import sistemaviaticosutd.reportes.generales.ReporteRelGastos;

/**
 *
 * @author jarteaga
 */
public class JIFReporteRelGastos extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("Reporte Relación Gastos");
        setToolTipText("");
    ...
    public void info_relacionGastos(Date fechaIn, Date fechaFin)
    {
        ...
        ResultSet obj=conexion.executeQuery("INSERT INTO relacion_gastos.relacion_gastos(fecha_generacion)VALUES('"+fecha_sistema+"')RETURNING consecutivo");
        try {
            while(obj.next())
            {
                id_rel_gastos=obj.getInt("consecutivo");
             ...
             * 
             */
         ResultSet obj1=conexion.executeQuery("SELECT MAX(det.fecha_despla)AS fecha, (to_char(current_date, 'yy') ||''||cta.id_cuenta_cobro)AS factura,(us.apellido_usuario ||' '|| us.nombre_usuario) AS usuario, SUM(desp.tarifa_despla) AS sumatotal, us.cedula_usuario AS nit "
                 ...
                 + "WHERE cta.cta_aprobada=true AND det.fecha_despla BETWEEN '"+fechaIn+"' AND '"+fechaFin+"' "
                 + "GROUP BY cta.id_cuenta_cobro,us.nombre_usuario,us.apellido_usuario,us.cedula_usuario "
                 + "ORDER BY cta.id_cuenta_cobro ASC");
               ...
               while(obj1.next())
               {
                   ...
                   conexion.executeUpdateQuery("INSERT INTO relacion_gastos.detalle_relacion_gastos(fecha,usuario,total_cta,factura,id_rel_gastos, nit_usuario)VALUES('"+fecha+"', '"+usuario+"', "+total_cta+", '"+factura+"', "+id_rel_gastos+", "+nit+")");
               }
            ...
            Logger.getLogger(JIFReporteRelGastos.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void relacion_gastos()
    {
        ...
        ResultSet obj=conexion.executeQuery("SELECT rel.consecutivo, rel.fecha_generacion, SUM(det.total_cta) AS total "
                + "FROM relacion_gastos.relacion_gastos rel JOIN relacion_gastos.detalle_relacion_gastos det ON rel.consecutivo=det.id_rel_gastos "
                + "GROUP BY rel.consecutivo ORDER BY rel.consecutivo ASC");
        try {
            while(obj.next())
            {
