...
import sistemaviaticosutd.reportes.tecnicos.ReporteTecnicos_21_1;

/**
 *
 * @author jarteaga
 */
public class JIFImprimirCtaViaticos extends javax.swing.JInternalFrame {
   ...
   // private ReporteTecnicos_1 reporte;
    ...
    private ReporteTecnicos_21_1 reporte3;
    
    public JIFImprimirCtaViaticos() {
        initComponents();
    ...
    private void btn_visuActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_visuActionPerformed
       ...
       // reporte.ejecutarReporte(idacta, id_manejo);
        reporte2.ejecutarReporte(idacta, id_manejo);
    ...
    private void btn_imprimirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_imprimirActionPerformed
        ...
        reporte3=new ReporteTecnicos_21_1();
        
       // reporte_1.ejecutarReporte(consecutivo, id_usuario);
        reporte3.ejecutarReporte(consecutivo, id_usuario);
    ...
    public void detalle_tabla()
    {
        ...
        conexion=new ConexionBD();
          ResultSet obj=conexion.executeQuery("SELECT 	cuenta,consecutivo,fecha,SUM(total) AS sumatotal "
                  + "FROM( "
                  + "SELECT cta.id_manejo_cta AS cuenta, cta.id_cuenta_cobro AS consecutivo,cta.fecha_sistema AS fecha, SUM(des.tarifa_despla)AS total "
                  ...
                  + "GROUP BY consecutivo,cuenta "
                  + "UNION  "
                  + "SELECT cta.id_manejo_cta AS cuenta, cta.id_cuenta_cobro AS consecutivo,cta.fecha_sistema AS fecha,SUM(co.tarifa)AS total "
                  + "FROM viaticos.otros_cobros co "
                  + "JOIN viaticos.cuenta_cobro cta ON co.id_cta_cobro=cta.id_manejo_cta "
                  + "JOIN usuarios.usuarios us ON us.id_usuario=cta.id_usuario "
                  + "WHERE cta.id_usuario="+id_usuario+" "
                  + "GROUP BY cta.id_cuenta_cobro,cta.id_manejo_cta) AS f "
                  + "GROUP BY cuenta,consecutivo,fecha "
                  + " ORDER BY fecha DESC");
        try {
          while (obj.next()) 
          {
          ...
          for (int i=0;i<4;i++)
          {
           ...
           modelo.addRow(datos);
            ...
            Logger.getLogger(JIFImprimirCtaViaticos.class.getName()).log(Level.SEVERE, null, ex);
        }
